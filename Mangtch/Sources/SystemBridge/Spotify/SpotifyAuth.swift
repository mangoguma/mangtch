...
final class SpotifyAuth: ObservableObject {
    ...
    private init() {
        // Reflect existing tokens at launch.
    ...
    /// Kick off the auth flow: open Spotify consent in the user's browser.
    /// Settings UI calls this when the user clicks "Sign in with Spotify".
    ...
    /// Called from SpotifyURLHandler when the OS routes the
    /// `mangtch://spotify-callback?code=…&state=…` URL back to us.
    func handleCallback(url: URL) {
        guard let components = URLComponents(url: url, resolvingAgainstBaseURL: false),
              let code = components.queryItems?.first(where: { $0.name == "code" })?.value,
              let state = components.queryItems?.first(where: { $0.name == "state" })?.value
        else {
            // Spotify error case: ?error=access_denied
        ...
        let clientID = SettingsManager.shared.spotifyClientID
        guard !clientID.isEmpty else {
    ...
    func currentAccessToken() async -> String? {
        ...
        // Try refresh.
        let clientID = await MainActor.run { SettingsManager.shared.spotifyClientID }
    ...
    private func refreshAccessToken(refreshToken: String, clientID: String) async -> String? {
        ...
        do {
            let (data, response) = try await URLSession.shared.data(for: request)
            guard let http = response as? HTTPURLResponse, http.statusCode == 200 else {
                // Refresh token revoked or expired — wipe and force re-auth.
            ...
            let resp = try JSONDecoder().decode(TokenResponse.self, from: data)
            // Spotify rotates refresh tokens; if a new one comes back, save it.
    ...
    /// RFC 7636-compliant 64-char URL-safe verifier.
    private static func generateCodeVerifier() -> String {
        var bytes = [UInt8](repeating: 0, count: 48)  // 64 chars after base64url
