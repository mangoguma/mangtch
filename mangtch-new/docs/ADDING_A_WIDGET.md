...
> Audience: anyone adding a new widget to mangtch-new (KBO/Music/Timer-class
...
- **Do not** wrap the body in a greedy `.frame(maxHeight:)` ceiling. SwiftUI
  ...
  of the actual content height. (See HANDOFF §0a Bug 3 for the postmortem.)
...
your widget files. The self-check greps in `PLAN-roadmap-7-to-10.md` §9
will surface them.
...
```bash
cd /Users/sarang/Projects/mangtch/mangtch-new && xcodebuild \
  -project boringNotch.xcodeproj -scheme boringNotch -configuration Release \
  -derivedDataPath build CODE_SIGN_IDENTITY="-" 2>&1 | tail -5
```

Then re-sign and install per the upstream README's WIP build instructions
(Team-ID mismatch with bundled `MediaRemoteAdapter.framework` requires
ad-hoc signing).
...
- `mangtch-new/PLAN-roadmap-7-to-10.md` — phase history (7 = tokens, 8 = width
  contract, 9a = two-axis ownership, 9c = ScrollView fallback)
- `mangtch-new/HANDOFF.md` — current open issues / recent bug postmortems
