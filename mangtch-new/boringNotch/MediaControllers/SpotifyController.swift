...
class SpotifyController: MediaControllerProtocol {
    ...
    /// Spotify's AppleScript dictionary has no usable favorited property
    /// (the legacy `starred` is read-only / no-op since Liked Songs replaced
    /// the old Star feature). The actual auth gate lives in MusicManager,
    /// which observes SpotifyAuth.isAuthorized and recomputes canFavoriteTrack
    /// when it flips — protocol can't gate per-call without async.
    ...
    /// Bare track ID extracted from the AppleScript `id` field
    /// (e.g., "3n3Ppam7vgaVa1iaRUc9Lp"). Used for Web API like-status calls.
    private var currentTrackID: String?
    /// Async task that fetches the liked status from the Web API. Cancelled
    /// when the track changes so we don't apply stale liked-state to a new song.
    private var likedFetchTask: Task<Void, Never>?
    ...
    func setFavorite(_ favorite: Bool) async {
        ...
        // Optimistically reflect the new liked state without a re-fetch.
    ...
    func updatePlaybackInfo() async {
        ...
        let trackIDRaw = descriptor.atIndex(11)?.stringValue ?? ""

        let newTrackID = SpotifyTrackID.extract(from: trackIDRaw)
        let trackChanged = newTrackID != currentTrackID
        currentTrackID = newTrackID

        // Carry the previous favorite state forward when the track is the
        // same; otherwise reset to false until the API confirms.
        let favoriteCarry = trackChanged ? false : playbackState.isFavorite
        ...
        if !artworkURL.isEmpty, let url = URL(string: artworkURL) {
            if artworkURL != lastArtworkURL || state.artwork == nil {
                artworkFetchTask?.cancel()
                ...
                artworkFetchTask = Task {
                    do {
                        ...
                        await MainActor.run { [weak self] in
                            ...
                            // Don't clobber a fresher liked-state that may
                            // have arrived between the artwork request and
                            // its completion.
                            updatedState.isFavorite = self.playbackState.isFavorite
        ...
        // Refresh liked-state from Web API. We always refresh because the
        // user may have liked the track in the Spotify app itself; the
        // 60s cache in SpotifyAPI keeps this from spamming the endpoint.
        if let trackID = newTrackID,
           await SpotifyAuth.shared.isAuthorized {
            likedFetchTask?.cancel()
            likedFetchTask = Task { [weak self] in
                let liked = await SpotifyAPI.shared.isLiked(trackID: trackID)
                await MainActor.run {
                    guard let self = self,
                          // Only apply if we're still on the same track —
                          // a track change between request and response
                          // would otherwise stamp stale liked-state.
                          self.currentTrackID == trackID,
                          self.playbackState.isFavorite != liked
                    else { return }
